...
class Orders extends Permissions
{
    public function index()
    {
        ...
        if (isset($post['keywords']) and !empty($post['keywords'])) {
            $where['order_sn|consignee|mobile'] = ['like', '%' . $post['keywords'] . '%'];
        }
        ...
        $orders = empty($where) ? $model->order('create_time desc')->paginate(20) : $model->where($where)->order('create_time desc')->paginate(20,false,['query'=>$this->request->param()]);

        //$articles = $article->toArray();
        //添加最后修改人的name
        foreach ($orders as $key => $value) {
            $orders[$key]['edit_admin'] = Db::name('admin')->where('id',$value['edit_admin_id'])->value('nickname');
        }
