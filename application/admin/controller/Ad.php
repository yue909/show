...
class Ad extends Permissions
{
    public function index(){       
        ...
        if($is_app_ad == 1){
            ...
            if($ad_info && $ad_info['media_type'] == 3){//如果广告类型是商品,则查找商品的名称 
               $ad_info['goods_name'] = db('goods')->where('goods_id' , $ad_info['ad_link'])->getField("goods_name");
        ...
        $position = db('ad_position')->select();
    ...
    public function adHandle(){
    	...
    	if($data['act'] == 'edit'){
    		$r = db('ad')->where('ad_id', $data['ad_id'])->save($data);
    	}
    	
    	if($data['act'] == 'del'){
            $r = db('ad')->where('ad_id', $data['del_id'])->delete();
            if($r){
